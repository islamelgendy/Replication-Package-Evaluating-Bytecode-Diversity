    ...
    with open(sigfile, "w") as sigfile:
        with open(input_file) as fin:
            ...
            for tc in fin:
                ...
                for hash_ in sig:
                    # tc_signature[i] = bytes.fromhex(tc_hash)
                    sigfile.write(repr(unpack('>d', bytes.fromhex(hash_))[0]))
                    sigfile.write(" ")
                sigfile.write("\n")
                tcID += 1
...
def fast_pw(input_file, r, b, bbox=False, k=5, memory=False):
    ...
    for i in range(n):
        if tcs_minhashes[first_tc][i] < bytes.fromhex(selected_tcs_minhash[i]):
            selected_tcs_minhash[i] = tcs_minhashes[first_tc][i].hex()
    ...
    while len(tcs_minhashes) > 0:
        ...
        for candidate in tcs_minhashes:
            if candidate in candidates:
                temp = selected_tcs_minhash.copy()
                for i in range(n):
                    temp[i] = bytes.fromhex(selected_tcs_minhash[i])
                dist = lsh.jDistanceEstimate(
                    temp, tcs_minhashes[candidate])
                if dist > max_dist:
                    selected_tc, max_dist = candidate, dist

        for i in range(n):
            if tcs_minhashes[selected_tc][i] < bytes.fromhex(selected_tcs_minhash[i]):
                selected_tcs_minhash[i] = tcs_minhashes[selected_tc][i].hex()
