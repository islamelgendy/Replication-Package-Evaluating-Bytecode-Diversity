...
def runExp(root, project, id, testType, totalcoverage = True, isBranchCoverage = False):
    ...
    reCoverageObj = CoverageSelection(statementCSV, totalStatementCoverage, foundation, branch=isBranchCoverage)# normalizedSetOfTests)
    ...
    raceIndex = 0
    coverageSize =  len(reCoverageObj.matrix)
    minSize = coverageSize
    coverageFlag = False
    
    totalNumTests = coverageSize                                # Total number of tests
    ...
    while len(reCoverageObj.matrixP) < minSize:
        iteration += 1
        if iteration % 100 == 0:
        ...
        if totalcoverage:
            coverageMethod = reCoverageObj.makeOneSelectionCoverage()
        else:
            coverageMethod = reCoverageObj.makeOneSelectionAdditionalCoverage()
        mh_time = time.process_time() - mh_t
        totalTime += mh_time
        # print(diversityObject.matrixP.keys())
        score4 = evaluateMutationScore(mutationCSV.matrix, reCoverageObj.matrixP.keys(), mutationCSV.listCols)
        
        if score4 == MS and not coverageFlag:
            coverageFlag = True
        ...
        updateFaultDetection(coverageMethod, fault_tests, probability, FD4, raceIndex, 'diversity', realfaultRace)
    ...
    # Calculate the APFDs
    APFD = list()
    APFD4 = calculateAPFD(reCoverageObj.matrixP, mutationCSV)

    print(f"Time is {totalTime} sec.")
    
    if len(realfaultRace) == 0:
        checkRaceDictionary(realfaultRace, len(reCoverageObj.matrixP))
...
if len(args) == 1:
    ...
    prj = args[1]
    totalCoverage = args[2]
    coverageType = 'Branch'
else: 
    print('Wrong number of arugments passed')
    exit()

if totalCoverage == 'True':
    totalCoverage = True
else:
    totalCoverage = False
# math projects:
...
for id in projects:
    # try:
        ...
        # runExp('math', id, '')
        da4, realFaultRace, mhtime = runExp(root, prj, id, 'all', totalCoverage, coverageType == 'Branch')
        print(f'APFD is: {da4}')
        times.append(mhtime)
