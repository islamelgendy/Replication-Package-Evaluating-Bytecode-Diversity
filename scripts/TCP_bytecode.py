...
def checkRaceDictionary(race, maxSize):
    if not 'bytecode' in race.keys():
        race['bytecode'] = maxSize
...
def runExp(root, project, id, testType, aftercoverage = True, isBranchCoverage = False):
    ...
    try:
        if testType == 'Randoop':
            similarityBytecodeFile = filterFiles(similarityFolder, project + '.Randoop.' + id + 'f', bytecodeExt)[0]
        elif testType == 'Evosuite':
            similarityBytecodeFile = filterFiles(similarityFolder, project + '.Evosuite.' + id + 'f', bytecodeExt)[0]
        elif testType == 'Developer':
            similarityBytecodeFile = filterFiles(similarityFolder, project + '.Developer.' + id + 'f', bytecodeExt)[0]
        else:    
            # similarityBytecodeFile = filterFiles(similarityFolder, project + '.all.' + id + 'f', bytecodeExt)[0]
            similarityBytecodeFile = filterFiles(similarityFolder, project + '.filter.' + id + 'f', bytecodeExt)[0]
    ...
    diversityBytecodeObject = DiversitySelection(simlarityBytecodeCSV, foundation)
    # separatehybridObject = SeparateHybridSelection(statementCSV, simlarityCSV, coverageObj.matrixP.keys())# normalizedSetOfTests)

    diversitySize =  len(diversityBytecodeObject.matrix) + len(diversityBytecodeObject.matrixP)
    ...
    totalNumTests = len(diversityBytecodeObject.matrix.keys())            # Total number of tests
    ...
    while len(diversityBytecodeObject.matrixP) < minSize:
        iteration += 1
        if iteration % 100 == 0:
        ...
        score4 = evaluateMutationScore(mutationCSV.matrix, diversityBytecodeObject.matrixP.keys(), mutationCSV.listCols)
        
        if score4 == MS and not bytecodeFlag:
            bytecodeFlag = True
            race['bytecode'] = raceIndex
        ...
        updateFaultDetection(bytecodeMethod, fault_tests, probability, FD4, raceIndex, 'bytecode', realfaultRace)
    ...
    # Calculate the APFDs
    APFD = list()
    APFD4 = calculateAPFD(diversityBytecodeObject.matrixP, mutationCSV)

    print(f"Time is {totalTime} sec.")
    # print(f"APFD values = {MS4}")
    
    if len(realfaultRace) == 0:
        checkRaceDictionary(realfaultRace, len(diversityBytecodeObject.matrixP))
...
for i in range (start, end):
    # try:
        ...
        # runExp('math', id, '')
        da4, realFaultRace, mhtime = runExp(root, prj, id, 'all', False, coverageType == 'Branch')
        ...
        bytecodeFDRace.append( realFaultRace['bytecode'])
